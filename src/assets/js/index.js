...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "Hola chat soy su fan", "author": "Darken" },
            { "message": "When haces eventos: hola", "author": "Darken" },
            { "message": "CausaNodes x Bridge!", "author": "Darken" }
        ];
    ...
    async maintenanceCheck() {
        config.GetConfig().then(res => {
            ...
            console.error(e);
            return this.shutdown("No se detectó conexión a internet,<br>Intenta de nuevo!!");
        })
    }

    startLauncher() {
        this.setStatus(`Iniciando el Cliente`);
        ipcRenderer.send('main-window-open');
        ipcRenderer.send('update-window-close');
    }

    shutdown(text) {
        this.setStatus(`${text}<br>Cerrando Cliente en 5s`);
        let i = 4;
        setInterval(() => {
            this.setStatus(`${text}<br>Adiós!! ${i--}s`);
